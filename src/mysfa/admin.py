...
class ChangeRequestAdmin(admin.ModelAdmin):
    list_display = ("id", "group", "kind", "status", "requester", "approver", "created_at", "submitted_at", "decided_at")
    list_filter = ("kind", "status", "group")
    search_fields = ("title", "note", "requester__custom_user_id", "requester__username", "group__name", "group__custom_id")


@admin.register(ChangeRequestRow)
class ChangeRequestRowAdmin(admin.ModelAdmin):
    list_display = ("id", "change_request", "row_index", "op", "code", "name", "is_valid")
